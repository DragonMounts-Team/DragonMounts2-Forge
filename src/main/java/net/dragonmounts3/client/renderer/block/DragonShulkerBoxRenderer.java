package net.dragonmounts3.client.renderer.block;
...
import static net.dragonmounts3.objects.blocks.BlockDragonShulkerBox.FACING;

/**
 * @see net.minecraft.client.renderer.tileentity.ShulkerBoxTileEntityRenderer
 */
@OnlyIn(Dist.CLIENT)
public class DragonShulkerBoxRenderer extends TileEntityRenderer<TileEntityDragonShulkerBox> {
    private static final ResourceLocation TEXTURE_LOCATION = DragonMounts.prefix("textures/blocks/dragon_shulker_box.png");
    private final ShulkerModel<?> model = new ShulkerModel<>();

    public DragonShulkerBoxRenderer(TileEntityRendererDispatcher dispatcher) {
        super(dispatcher);
    }

    @Override
    public void render(@Nonnull TileEntityDragonShulkerBox blockEntity, float partialTicks, @Nonnull MatrixStack matrixStack, @Nonnull IRenderTypeBuffer buffer, int combinedLight, int combinedOverlay) {
