 ...
 */
public class BlockDragonShulkerBox extends ContainerBlock {
    ...
    private static boolean testPositionPredicate(BlockState state, IBlockReader world, BlockPos pos) {
        ...
        if (tileentity instanceof TileEntityDragonShulkerBox) {
            return ((TileEntityDragonShulkerBox) tileentity).isClosed();
        }
        return true;
    }

    public BlockDragonShulkerBox() {
        super(Properties.of(Material.PORTAL, MaterialColor.COLOR_BLACK).strength(2000, 600).sound(SOUND).dynamicShape().noOcclusion().isSuffocating(BlockDragonShulkerBox::testPositionPredicate).isViewBlocking(BlockDragonShulkerBox::testPositionPredicate));
    ...
    public TileEntity newBlockEntity(@Nonnull IBlockReader world) {
        return new TileEntityDragonShulkerBox();
    }
    ...
    public ActionResultType use(@Nonnull BlockState state, @Nonnull World level, @Nonnull BlockPos pos, @Nonnull PlayerEntity player, @Nonnull Hand hand, @Nonnull BlockRayTraceResult hit) {
        if (level.isClientSide) {
            ...
            if (tileentity instanceof TileEntityDragonShulkerBox) {
                TileEntityDragonShulkerBox dragonshulkerbox = (TileEntityDragonShulkerBox) tileentity;
    ...
    public VoxelShape getShape(@Nonnull BlockState state, IBlockReader pLevel, @Nonnull BlockPos pos, @Nonnull ISelectionContext context) {
        ...
        return tileentity instanceof TileEntityDragonShulkerBox ? VoxelShapes.create(((TileEntityDragonShulkerBox) tileentity).getBoundingBox()) : VoxelShapes.block();
    }
