...
public class UndergroundDragonNestConfig extends DragonNestConfig {
    public static final Codec<UndergroundDragonNestConfig> CODEC = RecordCodecBuilder.create(builder -> builder.group(
    ...
    ).apply(builder, UndergroundDragonNestConfig::new));

    public UndergroundDragonNestConfig(double chance, List<ResourceLocation> structures) {
    ...
    public DragonNestPiece generatePiece(Random random, DynamicRegistries registries, ChunkGenerator generator, TemplateManager manager, int chunkX, int chunkY, Biome biome) {
        ...
        return new UndergroundDragonNestPiece(template, blockpos2, templateLocation, rotation, mirror, pivot);
    }
}
